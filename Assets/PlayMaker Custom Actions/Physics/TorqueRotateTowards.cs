...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class TorqueRotateTowards : FsmStateAction
	{
		...
		void DoRotateTowardsAt()
		{
			...
			go.GetComponent<Rigidbody>().maxAngularVelocity = maxSpeed.Value;

			Quaternion rotation = target.transform.rotation * Quaternion.Inverse(go.GetComponent<Rigidbody>().rotation);
			go.GetComponent<Rigidbody>().AddTorque(rotation.x * forceMultiplier.Value / Time.fixedDeltaTime, 
